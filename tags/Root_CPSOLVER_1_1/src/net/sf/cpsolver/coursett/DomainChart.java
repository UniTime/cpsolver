...
/**
 * Create domain chart of the given input problem as CSV file
 * (3 dimensions: #rooms, #times, #variables with the given number of rooms/times)
 * 
 * @version
...
public class DomainChart {
    ...
    public DomainChart(String name, TimetableModel model, int sizeX, int sizeY) {
        iModel = model; iName = name;
        iSizeX = sizeX; iSizeY = sizeY;
    }
    
    public DomainChart(File xmlFile, int sizeX, int sizeY) throws Exception {
        this(xmlFile.getName().substring(0, xmlFile.getName().lastIndexOf('.')),new TimetableModel(new DataProperties()),sizeX,sizeY);
    ...
    protected void computeTable() {
        ...
        for (Enumeration e=iModel.variables().elements();e.hasMoreElements();) {
            ...
            if (lecture.getNrRooms()>1)
                add(lecture.nrTimeLocations(),(int)Math.round(Math.pow(lecture.nrRoomLocations(),1.0/lecture.getNrRooms())),1);
            else
        ...
        try {
            ...
            if (args.length>3) { 
                ...
                if (output.exists() && output.isDirectory())
                    output = new File(output,input.getName().substring(0,input.getName().lastIndexOf('.'))+"_domain.csv");
            } else {
                output = new File(input.getParentFile(),input.getName().substring(0,input.getName().lastIndexOf('.'))+"_domain.csv");
            }
