...
/**
 * Export instructor direct, back-to-back, and more than two exams a day conflicts 
 * into a CSV file.
 * <br><br>
 * Usage:<br>
 * <code>
...
public class ExamInstructorConflicts {
    ...
    public CSVFile report() {
        ...
        for (Enumeration e=iModel.getInstructors().elements();e.hasMoreElements();) {
            ...
            for (Enumeration f=iModel.getPeriods().elements();f.hasMoreElements();) {
                ...
                if (nrExams>1) {
                    ...
                    for (Iterator i=instructor.getExams(period).iterator();i.hasNext();) {
                    ...
                    csv.addLine(new CSVField[] {
                            new CSVField(instructor.getName()),
                            new CSVField("direct"),
                ...
                if (nrExams>0) {
                    if (period.next()!=null && !instructor.getExams(period.next()).isEmpty() && (!iModel.isDayBreakBackToBack() || period.next().getDay()==period.getDay())) {
                        for (Iterator i=instructor.getExams(period).iterator();i.hasNext();) {
                            Exam ex1 = (Exam)i.next();
                            for (Iterator j=instructor.getExams(period.next()).iterator();j.hasNext();) {
                ...
                if (period.next()==null || period.next().getDay()!=period.getDay()) {
                    ...
                    if (nrExamsADay>2) {
                        ...
                        String periodTimes = "";
                        String rooms = "";
                        for (Iterator i=instructor.getExamsADay(period.getDay()).iterator();i.hasNext();) {
                            Exam exam = (Exam)i.next();
                            ExamPlacement placement = (ExamPlacement)exam.getAssignment();
                            String roomsThisExam = "";
                            for (Iterator k=placement.getRoomPlacements().iterator();k.hasNext();) {
