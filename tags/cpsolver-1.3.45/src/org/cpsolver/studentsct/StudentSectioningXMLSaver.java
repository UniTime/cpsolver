...
public class StudentSectioningXMLSaver extends StudentSectioningSaver {
        ...
        for (Offering offering : getModel().getOfferings()) {
            ...
            if (!offering.getReservations().isEmpty()) {
                for (Reservation r: offering.getReservations()) {
                    ...
                    reservationEl.addAttribute("priority", String.valueOf(r.getPriority()));
                    if (r.mustBeUsed()) reservationEl.addAttribute("mustBeUsed", "true");
                    if (r.isAllowOverlap()) reservationEl.addAttribute("allowOverlap", "true");
                    if (r.canAssignOverLimit()) reservationEl.addAttribute("canAssignOverLimit", "true");
    ...
    protected void saveReservation(Element reservationEl, Reservation reservation) {
        ...
        if (reservation.mustBeUsed()) reservationEl.addAttribute("mustBeUsed", "true");
        if (reservation.isAllowOverlap()) reservationEl.addAttribute("allowOverlap", "true");
        if (reservation.canAssignOverLimit()) reservationEl.addAttribute("canAssignOverLimit", "true");
