...
public class OnlineSectProof {
    ...
    public static class Sequence {
         ...
         * @param base base (e.g., 2 for a binary vector)
         */
        ...
        private boolean inc(int pos) {
            if (pos>=iSequence.length) return false;
            iCnt[iSequence[pos]]--;
            iSequence[pos] = (iSequence[pos]+1)%iBase;
            iCnt[iSequence[pos]]++;
            if (iSequence[pos]==0) return inc(pos+1);
            return true;
        }
        ...
        public void set(String seq) {
            for (int i=0;i<iCnt.length;i++)
                iCnt[i]=0;
            for (int i=0;i<iSequence.length;i++) {
                iSequence[i]=(int)(seq.charAt(i)-'A');
                iCnt[iSequence[i]]++;
            }
        }
        ...
        /** If a sequence of all zeros is considered as 0, and the highest possible sequence (sequence of all base-1) is 1, 
         * this returns the position of the current sequence between these two bounds. */
        public double progress() {
            double ret = 0.0;
            double mx = 1.0;
            for (int i=size()-1;i>=0;i--) {
                ret += mx * ((double)iSequence[i])/iBase;
                mx *= 1.0/iBase;
            }
            return ret;
        }
        
        /** Category of a sequence, i.e., a string representation of the count of each number in the sequence. E.g.,
         * A5B3C1 means that there are 5 zeros, 3 ones, and 1 two int the sequence.
         */
        public String cat() {
            StringBuffer s = new StringBuffer();
    ...
    /** Extension of {@link OnlineSectProof.Sequence} that represents an ordered set of students as they are to be enrolled into a course (given set of sections). */ 
    public static class StudentSequence extends Sequence {
        ...
        /** 
         * Constructor
         * @param columns limits of sections of a course (e.g., new int[] {5, 10} for two sections, first of the size 5, second of the size of 10) 
         */
        ...
        public boolean check() {
            for (int i=0;i<base();i++)
                if (maxCnt(i)<count(i)) return false;
            for (int c=0;c<nrColumns();c++) {
                int allowed = 0;
                for (int i=0;i<size() && allowed<limit(c);i++)
                    if (allow(seq(i), c)) allowed++;
                if (allowed<limit(c)) return false;
            }
            return true;
        }
         ...
         * Student type can be seen as a binary string that has 1 for each section into which a 
         * student can be enrolled. I.e., student of type 0 can be enrolled into the fist 
         * section, type 2 into the second section, type 3 into both first and section section, 
         * type 4 into the third section, type 5 into the first and third section etc.
         */
        public boolean allow(int x, int col) {
            return ((x+1) & (1<<col))!=0;
        }
        /**
         * Number of sections into which a student of a given type can be enrolled 
         * (see {@link OnlineSectProof.StudentSequence#allow(int, int)}).
         */
        ...
        /**
         * Maximum number of student of the given type that can be enrolled into the
         * provided sections (i.e., sum of limits of sections that are allowed fot the 
         * student of the given type, see {@link OnlineSectProof.StudentSequence#allow(int, int)}).
         */
        public int maxCnt(int x) {
            int ret = 0;
            for (int i=0;i<nrColumns();i++)
                if (allow(x,i)) ret += limit(i);
            return ret;
        }
    }
    
    /** Implemented online algorithms (heuristics) */
    public static String sOnlineAlgs[] = new String[] {
        "Max(Available)",
        "Min(Used/Limit)",
        "Min(Expected-Available)",
        "Min(Expected/Available)",
        "Min((Expected-Available)/Limit)",
        "Min(Expected/(Available*Limit))"
    };
    
    /**
     * Return true, if the given heuristics should be skipped (not evaluated).
     * @param computeExpectations true, if expected demand should be computed in advance
     * @param alg online algorithm (see {@link OnlineSectProof#sOnlineAlgs})
     * @param allTheSame true, if all the sections are of the same size
     * @return true if the given heuristics does not need to be computed (e.g., it is the same of some other)
     */
     ...
     * @param limit section limit
     * @param used number of space already used
     * @param expected expected demand for the given section
     * @param alg online algorithm (see {@link OnlineSectProof#sOnlineAlgs})
     * @return value that is to be minimized (i.e., a section with the lowest number will be picked for the student) 
     */
     ...
     * @param sq sequence of studends
     * @param computeExpectations true, if expected demand for each section should be computed in advance (otherwise, it is initially set to zero)
     * @param alg online algorithm (see {@link OnlineSectProof#sOnlineAlgs})
     * @param debug if true, some debug messages are printed
    ...
    public static int checkOnline(StudentSequence sq, boolean computeExpectations, int alg, boolean debug) {
        ...
        for (int i=0;i<sq.nrColumns();i++) { used[i]=0; exp[i]=0.0; }
        ...
        for (int i=0;i<sq.size();i++) {
            ...
            for (int c=0;c<sq.nrColumns();c++) {
                if (!sq.allow(x, c)) continue;
                if (used[c]>=sq.limit(c)) continue;
                double obj = onlineObjective(sq.limit(c),used[c], exp[c], alg);
                if (debug) System.out.println("      -- test "+((char)('A'+x))+" --> "+(c+1)+" (OBJ="+sDF.format(obj)+")");
                if (bestCol<0 || bestObj>obj) {
                    bestCol = c; bestObj = obj;
                }
            }
            if (bestCol>=0) {
                used[bestCol]++;
                ...
                if (debug) {
                    ...
                    System.out.println("    "+((char)('A'+x))+" --> "+(bestCol+1)+" (OBJ="+sDF.format(bestObj)+", USE:["+sbUse+"], EXP:["+sbExp+"])");
                }                
            } else {
                if (debug) System.out.println("    "+((char)('A'+x))+" --> FAIL");
                ret++;
            }
        }
        return ret;
    ...
    public static class Counter {
        ...
        public Counter(int init) { iCnt = init; }
    ...
    public static class CatCmp implements Comparator {
        Hashtable iWorstCaseCat, iTotalCat, iCountCat;
        /**
         * Constructor
         * @param countCat table (category, number of sequences of that category)
         * @param totalCat table (category, total number of students that were not sectioned of a sequence from this category)
         * @param worstCaseCat (category, worst number of students that were not sectioned of a sequence from this category)
         */
        public CatCmp(Hashtable countCat, Hashtable totalCat, Hashtable worstCaseCat) {
            iWorstCaseCat = worstCaseCat; iTotalCat = totalCat; iCountCat = countCat;
        }
        /**
         * Higher number of not-sectioned students in the worst case goes first.
         * If the same, higher average number of not-sectioned students goes first.
         * If the same, compare by category. 
         */
        public int compare(Object o1, Object o2) {
            String c1 = (String)o1;
            String c2 = (String)o2;
            int wc1 = ((Integer)iWorstCaseCat.get(c1)).intValue();
            int wc2 = ((Integer)iWorstCaseCat.get(c2)).intValue();
            int cmp = Double.compare(wc2, wc1);
            if (cmp!=0) return cmp;
            int cc1 = ((Counter)iCountCat.get(c1)).intValue();
            int tc1 = ((Counter)iTotalCat.get(c1)).intValue();
            int cc2 = ((Counter)iCountCat.get(c2)).intValue();
            int tc2 = ((Counter)iTotalCat.get(c2)).intValue();
     ...
     * @param args set of integers -- limits of each sections
     */
    public static void main(String[] args) {
        int[] x = new int[args.length];
        for (int i=0;i<args.length;i++)
            x[i] = Integer.parseInt(args[i]);
        if (args.length==0) x = new int[] {5, 5};
        ...
        for (int col = 0; col<x.length; col++) {
            ...
            if (sameSize<0) sameSize=sq.limit(col);
            else if (sameSize!=sq.limit(col)) allTheSameSize=false;
        }
        ...
        for (int i = 0; i<sq.base(); i++) {
            ...
            for (int col = 0; col<x.length; col++) {
                if (sq.allow(i, col)) System.out.println("      "+(col+1)+". column allowed"); 
            }
        }
        ...
        if (System.getProperty("check")!=null) {
            ...
            if (System.getProperty("case")!=null) {
                ...
                for (int i=0;i<2*sOnlineAlgs.length;i++) {
                    if (skip((i%2)==0, i/2, allTheSameSize)) continue;
                    System.out.println("Online sectioning #"+(i+1)+" "+sOnlineAlgs[i/2]+((i%2)==0?"":" w/o precomputed expectations"));
                    checkOnline(sq, (i%2)==0, i/2, true);
                }
            }
        ...
        Hashtable[] worstCaseSqCat = new Hashtable[2*sOnlineAlgs.length];
        Hashtable[] worstCaseCat = new Hashtable[2*sOnlineAlgs.length];
        Hashtable[] totalCat = new Hashtable[2*sOnlineAlgs.length];
        Hashtable[] countCat = new Hashtable[2*sOnlineAlgs.length];
        for (int i=0;i<2*sOnlineAlgs.length;i++) {
            total[i]=0; worstCase[i]=-1; worstCaseSq[i]=null;
            worstCaseSqCat[i]=new Hashtable();
            worstCaseCat[i]=new Hashtable();
            totalCat[i]=new Hashtable();
            countCat[i]=new Hashtable();
        }
        long nrCases = 0;
        ...
        do {
            ...
            if ((nc%1000000) == 0) {
                ...
                System.out.println("  "+sDF.format(100.0*progress)+"% done in "+sDF.format((mark-t0)/60000.0)+" min ("+sDF.format(exp/60000.0)+" min to go, hit "+sDF.format(100.0*hc/nc)+"%)");
            }
        } while (sq.inc());
        ...
        for (int i=0;i<2*sOnlineAlgs.length;i++) {
            if (skip((i%2)==0, i/2, allTheSameSize)) continue;
            System.out.println("Online sectioning #"+(i+1)+" "+sOnlineAlgs[i/2]+((i%2)==0?"":" w/o precomputed expectations"));
            System.out.println("  worst case: "+sDF.format((100.0*worstCase[i])/sq.size())+"% ("+worstCase[i]+" of "+sq.size()+", sequence "+worstCaseSq[i]+")");
            System.out.println("  average case: "+sDF.format((100.0*total[i])/(sq.size()*nrCases))+"%");
            sq.set((String)worstCaseSq[i].first());
            checkOnline(sq, (i%2)==0, i/2, true);
            TreeSet cats = new TreeSet(new CatCmp(countCat[i], totalCat[i], worstCaseCat[i]));
            cats.addAll(totalCat[i].keySet());
            for (Iterator j=cats.iterator();j.hasNext();) {
                String cat = (String)j.next();
                int cc = ((Counter)countCat[i].get(cat)).intValue();
                int tc = ((Counter)totalCat[i].get(cat)).intValue();
                int wc = ((Integer)worstCaseCat[i].get(cat)).intValue();
                String wcsq = (String)worstCaseSqCat[i].get(cat);
                System.out.println("  Category "+cat+" (size="+cc+")");
                System.out.println("    worst case: "+sDF.format((100.0*wc)/sq.size())+"% ("+wc+" of "+sq.size()+", sequence "+wcsq+")");
