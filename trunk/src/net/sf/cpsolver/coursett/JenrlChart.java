...
/**
 * Create joint enrollment chart of the given input problem as CSV file
 * (3 dimensions: 1st variable, 2nd variable, number of students in common)
 * 
 * @version
...
public class JenrlChart extends DomainChart {
    ...
    protected void computeTable() {
        clearTable();
        TreeSet vars = new TreeSet(new Comparator() {
           public int compare(Object o1, Object o2) {
               Lecture l1 = (Lecture)o1;
               Lecture l2 = (Lecture)o2;
        ...
        int x=1;
        for (Iterator i=vars.iterator();i.hasNext();x++) {
            Lecture l1 = (Lecture)i.next();
            if (x>iMax) continue;
            iHeader[x] = String.valueOf(l1.students().size());
            int y=1;
            for (Iterator j=vars.iterator();j.hasNext();y++) {
                Lecture l2 = (Lecture)j.next();
                if (y>iMax) continue;
                iTitle[y] = l2.getName();
                if (x>=y) continue;
        ...
        try {
            ...
            if (args.length>2) { 
                ...
                if (output.exists() && output.isDirectory())
                    output = new File(output,input.getName().substring(0,input.getName().lastIndexOf('.'))+"_jenrl.csv");
            } else {
                output = new File(input.getParentFile(),input.getName().substring(0,input.getName().lastIndexOf('.'))+"_jenrl.csv");
            }
