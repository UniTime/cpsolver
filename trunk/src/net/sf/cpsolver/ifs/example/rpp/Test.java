...
/**
 * RPP test.  It takes one argument -- property file with all the parameters.
 * It allows to execute given number of tests. The problem is loaded from prolog-based text files.
 * For description of RPP problem see {@link RPPModel}.
 * <br><br>
 * Description of the input problem files can be found at <a href='http://www.fi.muni.cz/~hanka/rpp/instances.html'>http://www.fi.muni.cz/~hanka/rpp/instances.html</a>. Each input problem (e.g., gen22.pl) has the following structure:<ul><code>
 * objects([<br>
 * &nbsp;&nbsp;object(<br>
 * &nbsp;&nbsp;&nbsp;&nbsp;name( rect1 ),<br>
 * &nbsp;&nbsp;&nbsp;&nbsp;size( [ 2, 1 ] ),<br>
 ...
 * <br>
 * MPP instances contain an extra file with the solution (e.g., gen22.solution), with the following structure<ul><code>
 ...
 * <br>
 * There is also a file (e.g., gen22.mpp) describing which input placements are to be prohibited (not that if the input placement is prohibited, it means that also all values with the same X or Y coordinate are prohibited). It has the following structure:<ul><code>
 * perturbation( 1, 0, [] ).<br>
 * perturbation( 2, 0, [] ).<br> 
 * ...<br>
 ...
 * ...<br>
 * </code></ul>
 * Stating that for instance in the first test with 4 perturbations the rectangles rect44, rect80, rect127 and rect153 will have their initial value prohibited.
 * <br><br>
 * Test's parameters:
 * <br>
 ...
 * <tr><td>RPP.NrTests</td><td>{@link Integer}</td><td>Number of tests to be executed for each input instance</td></tr>
 * <tr><td>Rpp.Min<br>Rpp.Max<br>Rpp.Step</td><td>{@link Integer}</td><td>In case of MPP: minimal, maximal number and increment of input perturbations. An instance is loaded and tested for each given number of input perturbations.</td></tr>
 * <tr><td>Rpp.Min<br>Rpp.Max</td><td>{@link Integer}</td><td>In case of initial problem: minimal and maximal number of the input problem. An instance is loaded and tested for each given number from RPP.Min to RPP.Max.</td></tr>
 * <tr><td>Rpp.ProblemWidth<br>Rpp.ProblemHeight</td><td>{@link Integer}</td><td>Width and height of the placement area.</td></tr>
 ...
 * <code>INCLUDE_REGEXP=general.ini;{rpp85|rpp90|rpp95|mpp22}.ini;{5min}.ini;{cbs|rw1|tabu20}.ini</code><br>
...
public class Test {
    ...
    private static Model loadModel(int mWidth, int mHeight, Vector objects, Vector assigned, Vector perturbations, int perb, int test) {
        try {
            ...
            m.addConstraint(c);
            for (Enumeration e = ((PrologFile.Term) ((PrologFile.Term)objects.elementAt(0)).getContent().elementAt(0)).getContent().elements(); e.hasMoreElements();) {
                PrologFile.Term object = (PrologFile.Term)e.nextElement();
            ...
            for (Enumeration e = ((PrologFile.Term) ((PrologFile.Term)assigned.elementAt(0)).elementAt(0)).getContent().elements(); e.hasMoreElements();) {
                PrologFile.Term assignment = (PrologFile.Term)e.nextElement();
                String name = assignment.elementAt(0).getText();
                name = name.substring(0, name.length() - 1);
                int x = assignment.elementAt(1).elementAt(0).toInt();
                assignment = (PrologFile.Term)e.nextElement();
            ...
            for (Enumeration e = perturbations.elements(); e.hasMoreElements();) {
                PrologFile.Term pert = (PrologFile.Term)e.nextElement();
                if (test == pert.elementAt(0).toInt() && perb == pert.elementAt(1).toInt() && perb > 0) {
                    for (Enumeration x = pert.elementAt(2).getContent().elements(); x.hasMoreElements();) {
                        int rec = ((PrologFile.Term)x.nextElement()).toInt();
    ...
    private static Model loadModel(int mWidth, int mHeight, Vector objects) {
        try {
    ...
    private static void testMPP(DataProperties properties) {
        try {
            ...
            Vector v1 = PrologFile.readTermsFromStream(fis, "objects");
            fis.close();
            fis = new FileInputStream(properties.getProperty("General.Input") + ".solution");
            Vector v2 = PrologFile.readTermsFromStream(fis, "assigned");
            fis.close();
            fis = new FileInputStream(properties.getProperty("General.Input") + ".mpp");
            Vector v3 = PrologFile.readTermsFromStream(fis, "perturbation");
            fis.close();
            ...
            for (int i = min; size == -1 || i <= (max > 0 ? Math.min(max, size) : size); i += step) {
                ...
                for (int t = 1; t <= tests; t++) {
                    Model m = loadModel(properties.getPropertyInt("Rpp.ProblemWidth", 40), properties.getPropertyInt("Rpp.ProblemHeight", 14), v1, v2, v3, i, t);
                    if (size < 0) size = m.variables().size();
                    ...
                    for (Enumeration it = notPlaced.elements(); it.hasMoreElements();) {
                        Rectangle rect = (Rectangle)it.nextElement();
                        res.print(rect.getName() + "X," + rect.getName() + "Y" + (it.hasMoreElements() ? "," : ""));
                    }
                    res.println("]),");
                    StringBuffer sb = new StringBuffer();
                    int perts = 0;
                    for (Enumeration it = best.getModel().variables().elements(); it.hasMoreElements();) {
                        Rectangle rect = (Rectangle)it.nextElement();
                        if (rect.getBestAssignment() != null && (rect.getInitialAssignment() == null || !rect.getBestAssignment().equals(
                ...
                txt.println(i + ";" + nf.format(time / tests) + ";" + nf.format(ToolBox.rms(tests, time, time2)) + ";" + nf.format(((double)assigned) / tests) + ";" + 
                    nf.format(ToolBox.rms(tests, (double)assigned, (double)assigned2)) + ";" + 
                    nf.format(((double)perturbation) / tests) + ";" + 
                    nf.format(ToolBox.rms(tests, (double)perturbation, (double)perturbation2)) + ";" +
                    nf.format(((double)iters) / tests) + ";" + 
                    nf.format(ToolBox.rms(tests, (double)iters, (double)iters2)));
                txt.flush();
                stat.println("averages( initperturbations( " + i + " ), time( " +  nf.format(time / tests)+ " ), assigned( " + 
                    nf.format(((double)assigned) / tests) + " ), perturbations( " + nf.format(((double)perturbation) / tests) + " ) ).");
                stat.println("deviations( initperturbations( " + i + " ), time( " + 
                    nf.format(ToolBox.rms(tests, (double)time, (double)time2)) + " ), assigned( " + 
                    nf.format(ToolBox.rms(tests, (double)assigned, (double)assigned2)) + " ), perturbations( " + 
                    nf.format(ToolBox.rms(tests, (double)perturbation, (double)perturbation2)) + " ) ).");
                stat.flush();
            }
            res.close();
    ...
    private static void test(DataProperties properties) {
        try {
            ...
            for (int genNr = min; genNr <= max; genNr++) {
                ...
                long iters2 = 0;
                for (int t = 1; t <= tests; t++) {
                    Model m = loadModel(properties.getPropertyInt("Rpp.ProblemWidth", 40), properties.getPropertyInt("Rpp.ProblemHeight", 14), v1);
                    Solver s = new Solver(properties);
                    s.setInitalSolution(m);
                    s.start();
                    s.getSolverThread().join();
                ...
                txt.println(genNr + ";" + nf.format(time / tests) + ";" + nf.format(ToolBox.rms(tests, time, time2)) + ";" + nf.format(((double)assigned) / tests) + ";" + 
                    nf.format(ToolBox.rms(tests, (double)assigned, (double)assigned2)) + ";" + 
                    nf.format(((double)iters) / tests) + ";" + 
                    nf.format(ToolBox.rms(tests, (double)iters, (double)iters2)));
                txt.flush();
    ...
    private static void test(File inputCfg, String name, String include, String regexp, String outDir) throws Exception {
        if (regexp != null) {
            ...
            if (incFile.indexOf('{') >= 0 && incFile.indexOf('}') >= 0) {
                ...
                new StringTokenizer(incFile.substring( incFile.indexOf('{') + 1, incFile.indexOf('}')), "|");
    ...
    public static void main(String[] args) {
        try {
            Progress.getInstance().addProgressListener(
            new ProgressWriter(System.out));
