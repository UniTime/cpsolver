 ...
 * <br><br>
 * The problem is modelled in such a way that every lecture was represented by a variable, resource as a constraint
 * and every possible location of an activity in the time and space was represented by a single value. It means that a
 * value stands for a selection of the time (starting time slot), and one of the available rooms. Binary dependencies
 * are of course represented as constraints as well.
 *
 * @version
...
public class TimetableModel extends Model {
    ...
    public TimetableModel(int nrDays, int nrHours) {
        super();
        iNrDays = nrDays; iNrHours = nrHours;
    }
    
    public int getNrDays() { return iNrDays; }
    public int getNrHours() { return iNrHours; }
    ...
    public static TimetableModel generate(DataProperties cfg) {
        ...
        Vector groupForRoom[] = new Vector[nrRooms];
        for (int i=0;i<nrRooms;i++) {
            rooms[i] = new Resource("r"+(i+1),Resource.TYPE_ROOM, "Room "+(i+1));
            groupForRoom[i] = new Vector();
        ...
        for (int i=0;i<nrRooms;i++) {
            ...
            for (int j=0;j<nrGroupsOfRooms;j++)
                if (groupOfRooms[j].contains(rooms[i])) cnt++;
        ...
        while ((((double)usedSlots/((double)totalSlots)))<fillFactor) {
            ...
            Vector roomGr = (Vector)ToolBox.random(groupForRoom[room]);
            act++;
            usedSlots+=len;
            ...
            m.addVariable(a);
            starts.addElement(new Integer(slot));
            arooms.addElement(new Integer(room));
        ...
        for (int i=0;i<m.variables().size();i++) {
            Activity activity = (Activity)m.variables().elementAt(i);
            for (int slot = 0; slot < nrSlots; slot++) {
                int start = ((Integer)starts.elementAt(i)).intValue();
        ...
        for (int i=0;i<nrDependencies;) {
            ...
            int s1 = ((Integer)starts.elementAt(ac1)).intValue();
            int s2 = ((Integer)starts.elementAt(ac2)).intValue();
            Activity a1 = (Activity)m.variables().elementAt(ac1);
            Activity a2 = (Activity)m.variables().elementAt(ac2);
            Dependence dep = null;
            if (s1<s2) {
        ...
        for (int i=0;i<m.variables().size();i++) {
            ...
            int start = ((Integer)starts.elementAt(i)).intValue();
            int room = ((Integer)arooms.elementAt(i)).intValue();
            ...
            if (location!=null) {
                Set conflicts = m.conflictValues(location);
                if (!conflicts.isEmpty()) {
                    sLogger.warn("Unable to assign "+location.getName()+" to "+activity.getName()+", reason:");
                    for (Enumeration e=activity.constraints().elements();e.hasMoreElements();) {
                        Constraint c = (Constraint)e.nextElement();
                        Set cc = new HashSet(); c.computeConflicts(location, cc);
                        if (!cc.isEmpty())
    ...
    public void saveAsXML(DataProperties cfg, boolean gen, Solution solution, File outFile) throws IOException {
        ...
        if (!assignedVariables().isEmpty()) {
            ...
            Dictionary solutionInfo=(solution==null?getInfo():solution.getInfo());
            ...
            if (c instanceof Resource) {
                ...
                switch (r.getType()) {
                    ...
                    case Resource.TYPE_CLASS:
                        classes.addElement(r);
                        break;
                    case Resource.TYPE_INSTRUCTOR:
                        instructors.addElement(r);
                        break;
                    default:
                        specials.addElement(r);
                }
        ...
        for (Enumeration e=rooms.elements();e.hasMoreElements();) {
            ...
            for (Enumeration i=ToolBox.sortEnumeration((new Vector(r.getDiscouragedSlots())).elements());i.hasMoreElements();)
                pref.addElement("Soft").setText(i.nextElement().toString());
            for (Enumeration i=ToolBox.sortEnumeration((new Vector(r.getProhibitedSlots())).elements());i.hasMoreElements();)
                pref.addElement("Hard").setText(i.nextElement().toString());
        }
        ...
        for (Enumeration e=specials.elements();e.hasMoreElements();) {
            Resource r = (Resource)e.nextElement();
            Element el = resEl.addElement("Resource");
            el.addAttribute("id", r.getResourceId());
            el.addElement("Name").setText(r.getName());
        ...
        for (Enumeration e=variables().elements();e.hasMoreElements();) {
            ...
            if (a.getAssignment()!=null) hasSolution=true;
            Element pref = el.addElement("TimePreferences");
            for (Enumeration i=ToolBox.sortEnumeration((new Vector(a.getDiscouragedSlots())).elements());i.hasMoreElements();)
                pref.addElement("Soft").setText(i.nextElement().toString());
            for (Enumeration i=ToolBox.sortEnumeration((new Vector(a.getProhibitedSlots())).elements());i.hasMoreElements();)
                pref.addElement("Hard").setText(i.nextElement().toString());
            Element reqRes = el.addElement("RequiredResources");
            for (Enumeration f=a.getResourceGroups().elements();f.hasMoreElements();) {
                Vector gr = (Vector)f.nextElement();
                if (gr.size()==1) {
                    reqRes.addElement("Resource").setText(((Resource)gr.firstElement()).getResourceId());
                } else {
                    Element grEl = reqRes.addElement("Group").addAttribute("conjunctive","no");
                    for (Enumeration g=gr.elements();g.hasMoreElements();)
                        grEl.addElement("Resource").setText(((Resource)g.nextElement()).getResourceId());
                }
            }
        }
        ...
        for (Enumeration e=dependencies.elements();e.hasMoreElements();) {
            Dependence d = (Dependence)e.nextElement();
            Element el = depEl.addElement("Dependence");
            el.addAttribute("id", d.getResourceId());
            el.addElement("FirstActivity").setText(((Activity)d.first()).getActivityId());
            el.addElement("SecondActivity").setText(((Activity)d.second()).getActivityId());
            switch (d.getType()) {
        ...
        if (hasSolution) {
            ...
            for (Enumeration e=variables().elements();e.hasMoreElements();) {
                ...
                if (a.getAssignment()!=null) {
                    Location location = (Location)a.getAssignment();
    ...
    public static TimetableModel loadFromXML(File inFile, boolean assign) throws IOException, DocumentException {
        ...
        TimetableModel m = new TimetableModel(
                Integer.parseInt(problemGen.elementText("DaysPerWeek")),
        ...
        Hashtable actTab = new Hashtable();
        for (Iterator i=actEl.elementIterator("Activity");i.hasNext();) {
            Element el = (Element)i.next();
            Activity a = new Activity(Integer.parseInt(el.elementText("Length")), el.attributeValue("id"), el.elementText("Name"));
            Element pref = el.element("TimePreferences");
            for (Iterator j=pref.elementIterator("Soft");j.hasNext();)
                ...
                if ("Resource".equals(rqEl.getName())) {
                    ...
                    if ("no".equalsIgnoreCase(rqEl.attributeValue("conjunctive")) || "false".equalsIgnoreCase(rqEl.attributeValue("conjunctive"))) {
                        ...
                        for (Iterator k=rqEl.elementIterator("Resource");k.hasNext();)
                            gr.addElement(resTab.get(((Element)k.next()).getText()));
        ...
        for (Iterator i=depEl.elementIterator("Dependence");i.hasNext();) {
            ...
            d.addVariable((Activity)actTab.get(el.elementText("FirstActivity")));
            d.addVariable((Activity)actTab.get(el.elementText("SecondActivity")));
            m.addConstraint(d);
        }
        ...
        if (solEl!=null) {
            for (Iterator i=solEl.elementIterator("Activity");i.hasNext();) {
                Element el = (Element)i.next();
                Activity a = (Activity)actTab.get(el.attributeValue("id"));
                if (a==null) continue;
                ...
                for (Iterator j=usResEl.elementIterator("Resource");j.hasNext();)
                    res.addElement(resTab.get(((Element)j.next()).getText()));
                for (Enumeration e=a.values().elements();e.hasMoreElements();) {
                    ...
                    for (int j=0;j<loc.getResources().length && same;j++)
                        if (!res.elementAt(j).equals(loc.getResources()[j])) same=false;
                    if (!same) continue;
                    a.setInitialAssignment(loc);
                    if (assign) a.assign(0,loc);
                    break;
                }
            }
        }
