 ...
 * <br><br>
 * Many course offerings consist of multiple classes, with students enrolled in the
 * course divided among them. These classes are often linked by a set of constraints,
 * namely:<ul>
 * <li>Each class has a limit stating the maximum number of students who can be
 * enrolled in it.
 * <li>A student must be enrolled in exactly one class for each subpart of a course.
 * <li>If two subparts of a course have a parent–child relationship, a student enrolled
 * in the parent class must also be enrolled in one of the child classes.
 * </ul>
 * Moreover, some of the classes of an offering may be required or prohibited for
 * certain students, based on reservations that can be set on an offering, a configuration,
 * or a class.
 * <br>
 * Before implementing the solver, an initial sectioning of students into classes
 * is processed. This sectioning is based on Carter’s homogeneous sectioning
 * and is intended to minimize future student conflicts. However, it is still possible
 * to improve on the number of student conflicts in the solution. This can be accomplished
 * by moving students between alternative classes of the same course
 * during or after the search (see {@link FinalSectioning}). 
 * 
 * @version
...
public class InitialSectioning {
    Collection iStudents = null;
    Group[] iGroups = null;
    Long iOfferingId = null;
    Progress iProgress = null;
    
    public InitialSectioning(Progress progress, Long offeringId, Collection lectureOrConfigurations, Collection students) {
    	iOfferingId = offeringId;
    	iStudents = new HashSet(students);
    	iProgress = progress;
        ...
        for (idx=0;idx<iGroups.length;idx++)
        	progress.trace("    "+(idx+1)+". group can accomodate <"+iGroups[idx].getMinSize()+","+iGroups[idx].getMaxSize()+"> students");
    }
    ...
    private boolean moveAwayOneStudent(Group group) {
        ...
        for (Iterator i=group.getStudents().iterator();i.hasNext();) {
            ...
            for (int x=0;x<iGroups.length;x++) {
                if (group.equals(iGroups[x])) continue;
                if (iGroups[x].size()>iGroups[x].getMaxSize()) continue;
                if (!iGroups[x].canEnroll(student)) continue;
                double nd = iGroups[x].getDistance(student);
                if (newGroup==null || newDist-curDist<nd-cd) {
                    newGroup = iGroups[x]; movingStudent = student; curDist = cd; newDist = nd;
                    if (newDist-curDist<0.01) break;
                }
            }
        }
    ...
    public boolean moveIntoOneStudent(Group group) {
        ...
        for (int x=0;x<iGroups.length;x++) {
            if (group.equals(iGroups[x])) continue;
            if (iGroups[x].size()<=iGroups[x].getMinSize()) continue;
            for (Iterator i=iGroups[x].getStudents().iterator();i.hasNext();) {
                Student student = (Student)i.next();
                if (!group.canEnroll(student)) continue;
                if (iGroups[x].isEnrolled(student)) continue;
                double cd = iGroups[x].getDistance(student);
                double nd = group.getDistance(student);
                if (oldGroup==null || newDist-curDist<nd-cd) {
                    oldGroup = iGroups[x]; movingStudent = student; curDist = cd; newDist = nd;
                    if (newDist-curDist<0.01) break;
                }
            }
        }
    ...
    public Group[] getGroups() {
        ...
        for (Iterator i=iStudents.iterator();i.hasNext();) {
            ...
            Group group = null; double dist = 0.0;
            for (int idx=0;idx<iGroups.length;idx++) {
            	Group g = iGroups[idx];
            	if (!g.canEnroll(student)) continue;
            	if (g.size()+studentWeight>g.getMaxSize()) continue;
            ...
            iProgress.debug("Unable to find a valid section for student "+student.getId()+", enrolling to "+(group.getLecture()!=null?group.getLecture().getName():group.getConfiguration().getConfigId().toString()));
        ...
        for (int idx=0; idx<iGroups.length; idx++) {
        	...
        	while (group.size()>group.getMaxSize()) {
                if (!moveAwayOneStudent(group)) break;
            }
                ...
                for (Iterator i=group.getStudents().iterator();i.hasNext();) {
                    ...
                    for (int x=0;x<iGroups.length;x++) {
                        ...
                        while (iGroups[x].size()+student.getOfferingWeight(iOfferingId)>iGroups[x].getMaxSize()) {
                            if (!moveAwayOneStudent(iGroups[x])) break;
                        }
                        if (iGroups[x].size()+student.getOfferingWeight(iOfferingId)>iGroups[x].getMaxSize()) continue;
                        newGroup = iGroups[x];
        	...
        	while (group.size()<group.getMinSize()) {
        		if (!moveIntoOneStudent(group)) break;
        	}
        }
    ...
    public class Group {
        ...
        private Hashtable iDistCache = new Hashtable();
        ...
        public Configuration getConfiguration() { return iConfiguration; }
        public Lecture getLecture() { return iLecture; }
        public double getMinSize() { return iMinSize; }
        public double getMaxSize() { return iMaxSize; }
        public void setMinSize(double minSize) { iMinSize = minSize; }
        public void setMaxSize(double maxSize) { iMaxSize = maxSize; }
        ...
        public double getDistance() {
        	if (iDist==null) {
            	...
            	for (Iterator i=iStudents.iterator();i.hasNext();) {
            		...
            		if (Math.random()<prob) {
            			for (Iterator j=iStudents.iterator();j.hasNext();) {
            				Student s2 = (Student)j.next();
            				if (s1.getId().compareTo(s2.getId())<=0) continue;
            				if (Math.random()<prob) {
        ...
        public double getDistance(Student student) {
        	Double cachedDist = (Double)iDistCache.get(student);
        	if (cachedDist!=null) return cachedDist.doubleValue();
            double dist = 0.0;
            double prob = 10.0 / iStudents.size();
            int cnt = 0;
            for (Iterator i=iStudents.iterator();i.hasNext();) {
        ...
        public double size() { return iSize; }

        public String toString() { return "{"+size()+"-"+getDistance()+"/"+getStudents()+"}"; }
        
        public boolean canEnroll(Student student) {
        	if (getLecture()!=null) {
        		return student.canEnroll(getLecture());
        	} else {
        		for (Enumeration e=getConfiguration().getTopSubpartIds();e.hasMoreElements();) {
        			Long subpartId = (Long)e.nextElement();
        			boolean canEnrollThisSubpart = false;
        			for (Iterator i=getConfiguration().getTopLectures(subpartId).iterator();i.hasNext();) {
        				Lecture lecture = (Lecture)i.next();
        				if (student.canEnroll(lecture)) {
        					canEnrollThisSubpart = true; break;
        				}
        			}
        			if (!canEnrollThisSubpart) return false;
        		}
        		return true;
        	}
        }
    ...
    public static void initialSectioningCfg(Progress p, Long offeringId, String courseName, Collection students, Vector configurations) {
    	if (students==null || students.isEmpty()) return;
        if (configurations==null || configurations.isEmpty()) return;
    	if (configurations.size()==1) {
    		Configuration cfg = (Configuration)configurations.firstElement();
            ...
            for (int i=0;i<configurations.size();i++) {
                Group group = (Group)studentsPerSection[i];
        		...
        		for (Iterator j=group.getStudents().iterator();j.hasNext();) {
        			Student st = (Student)j.next();
        			st.addConfiguration(group.getConfiguration());
        		}
                for (Enumeration e=group.getConfiguration().getTopSubpartIds();e.hasMoreElements();) {
                	Long subpartId = (Long)e.nextElement();
                	initialSectioning(p, offeringId, courseName, group.getStudents(), group.getConfiguration().getTopLectures(subpartId));
                }
            }
    	}
    }
    ...
    private static void initialSectioning(Progress p, Long offeringId, String parentName, Collection students, Collection lectures) {
        ...
        if (lectures.size()==1) {
        	Lecture lect = (Lecture)lectures.iterator().next();
            ...
            if (lect.hasAnyChildren()) {
        		for (Enumeration e1=lect.getChildrenSubpartIds();e1.hasMoreElements();) {
        			Long subpartId = (Long) e1.nextElement();
                	Vector children = lect.getChildren(subpartId);
            ...
            for (int i=0;i<studentsPerSection.length;i++) {
                ...
                p.trace("Lecture "+getClassLabel(lect)+" got "+group.getStudents().size()+" students (weighted="+group.size()+", classLimit="+lect.classLimit()+")");
                Collection studentsThisSection = group.getStudents();
                for (Iterator j=studentsThisSection.iterator();j.hasNext();) {
                    Student st = (Student)j.next();
                    if (!st.canEnroll(lect)) {
                    	p.info("Unable to enroll student "+st.getId()+" in class "+getClassLabel(lect));
                    }
