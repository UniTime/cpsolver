 ...
 * <tr><td>Comparator.DeptSpreadPenaltyWeight</td><td>{@link Double}</td><td>Department balancing penalty (see {@link net.sf.cpsolver.coursett.constraint.DepartmentSpreadConstraint})</td></tr>
 * </table>
 *
 *
 * @version
...
public class TimetableComparator implements SolutionComparator {
    ...
    public double currentValue(TimetableModel tm, PerturbationsCounter cnt) {
        ...
        double prefCurr = (iEmptySingleSlotWeight*uselessSlotsCur)+
                       ...
                       (iSpreadPenaltyWeight*spread)+
                       (iCommitedStudentConflictWeight*commitedStudentConflicts)
                       ;
        return prefCurr;
    }
    
    public double currentValue(TimetableModel tm, PerturbationsCounter cnt, Vector variables) {
    	int unassigned = 0;
        ...
        double prefCurr = (iEmptySingleSlotWeight*uselessSlots)+
                       (iTimePreferencesWeight*timePref)+
                       (iRoomPreferencesWeight*roomPref)+
                       (iConstrPreferencesWeight*grPref)+
                       (iStudentConflictWeight*allSC)+
                       (iHardStudentConflictWeight*hardSC)+
                       (iTooBigRoomWeight*tooBigRooms)+
                       (iDistanceInstructorPreferenceWeight*instPref)+
                       (iPerturbationPenaltyWeight*pertCurr)+
                       (iDeptSpreadPenaltyWeight*deptSpreadPen)+
                       (iSpreadPenaltyWeight*spreadPen)+
                       (iCommitedStudentConflictWeight*comSC)
                       ;
        return prefCurr;
    }
    ...
    public double value(Placement placement, PerturbationsCounter cnt) {
    	...
    	return
    		(iEmptySingleSlotWeight==0.0?0.0:iEmptySingleSlotWeight*placement.nrUselessHalfHours())+
    		(iTimePreferencesWeight==0.0?0.0:iTimePreferencesWeight*placement.getTimeLocation().getNormalizedPreference())+
    		...
    		(iStudentConflictWeight==0.0?0.0:iStudentConflictWeight*lecture.countStudentConflicts(placement))+
    		(iHardStudentConflictWeight==0.0?0.0:iHardStudentConflictWeight*lecture.countHardStudentConflicts(placement))+
    		...
    		(iPerturbationPenaltyWeight==0.0 || cnt==null?0.0:iPerturbationPenaltyWeight*cnt.getPerturbationPenalty(lecture.getModel(),placement,new Vector(0)))+
    		(iDeptSpreadPenaltyWeight==0.0 || lecture.getDeptSpreadConstraint()==null?0.0:iDeptSpreadPenaltyWeight*lecture.getDeptSpreadConstraint().getPenalty(placement))+
    		(iSpreadPenaltyWeight==0.0?0.0:iSpreadPenaltyWeight*placement.getSpreadPenalty())+
    		(iCommitedStudentConflictWeight==0.0?0.0:iCommitedStudentConflictWeight*lecture.getCommitedConflicts(placement))
        ;
    }
}
