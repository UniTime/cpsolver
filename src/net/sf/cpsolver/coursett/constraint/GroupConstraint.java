 ...
 * <table border='1'><tr><th>Constraint</th><th>Comment</th></tr>
...
public class GroupConstraint extends Constraint {
    ...
    public void addVariable(Variable variable) {
    	if (!variables().contains(variable)) super.addVariable(variable);
    	if (isChildrenNotOverlap(getType())) {
    		Lecture lecture = (Lecture)variable;
    		if (lecture.getChildrenSubpartIds()!=null) {
            	for (Enumeration e1=lecture.getChildrenSubpartIds();e1.hasMoreElements();) {
            		Long subpartId = (Long)e1.nextElement();
            		for (Enumeration e2=lecture.getChildren(subpartId).elements();e2.hasMoreElements();) {
            			Lecture ch = (Lecture)e2.nextElement();
            			if (!variables().contains(ch)) super.addVariable(ch);
            		}
            	}
    		}
    	}
    ...
    public void removeVariable(Variable variable) {
    	if (variables().contains(variable)) super.removeVariable(variable);
    	if (isChildrenNotOverlap(getType())) {
    		Lecture lecture = (Lecture)variable;
    		if (lecture.getChildrenSubpartIds()!=null) {
            	for (Enumeration e1=lecture.getChildrenSubpartIds();e1.hasMoreElements();) {
            		...
            		for (Enumeration e2=lecture.getChildren(subpartId).elements();e2.hasMoreElements();) {
            			Lecture ch = (Lecture)e2.nextElement();
            			if (variables().contains(ch)) super.removeVariable(ch);
            		}
            	}
    		}
    	}
     ...
     * @param type constraString type (e.g, "SAME_TIME")
    ...
    public long getId() { return (iId==null?-1:iId.longValue()); }
    ...
    public String getPrologPreference() { return Constants.preferenceLevel2preference(iPreference); }
    ...
    private static int getGapMin(String type) {
        if (type.equals(TYPE_BTB)) return 0;
        else if (type.equals(TYPE_BTB_TIME)) return 0;
        else if (type.equals(TYPE_NHB_1)) return 6*2;
        else if (type.equals(TYPE_NHB_1_5)) return 6*3;
        else if (type.equals(TYPE_NHB_2)) return 6*4;
        else if (type.equals(TYPE_NHB_3)) return 6*6;
        else if (type.equals(TYPE_NHB_4)) return 6*8;
        else if (type.equals(TYPE_NHB_4_5)) return 6*9;
        else if (type.equals(TYPE_NHB_5)) return 6*10;
        else if (type.equals(TYPE_NHB_6)) return 6*12;
        else if (type.equals(TYPE_NHB_7)) return 6*14;
        else if (type.equals(TYPE_NHB_8)) return 6*16;
        else if (type.equals(TYPE_NHB_GTE_1)) return 6*1;
        else if (type.equals(TYPE_NHB_LT_6)) return 0;
        return -1;
    }
    ...
    private static int getGapMax(String type) {
        ...
        else if (type.equals(TYPE_NHB_GTE_1)) return Constants.SLOTS_PER_DAY;
        else if (type.equals(TYPE_NHB_LT_6)) return 6*11;
        return -1;
    }
    
    private static boolean isBackToBack(String type) {
        if (type.equals(TYPE_BTB)) return true;
        if (type.equals(TYPE_BTB_TIME)) return true;
        if (type.equals(TYPE_NHB_1)) return true;
        if (type.equals(TYPE_NHB_1_5)) return true;
        if (type.equals(TYPE_NHB_2)) return true;
        if (type.equals(TYPE_NHB_3)) return true;
        if (type.equals(TYPE_NHB_4)) return true;
        if (type.equals(TYPE_NHB_4_5)) return true;
        if (type.equals(TYPE_NHB_5)) return true;
        if (type.equals(TYPE_NHB_6)) return true;
        if (type.equals(TYPE_NHB_7)) return true;
        if (type.equals(TYPE_NHB_8)) return true;
        if (type.equals(TYPE_NHB_GTE_1)) return true;
        if (type.equals(TYPE_NHB_LT_6)) return true;
        return false;
    }
    ...
    private static boolean sameRoom(String type) {
        if (type.equals(TYPE_SAME_ROOM)) return true;
        if (type.equals(TYPE_MEET_WITH)) return true;
        return false;
    }
    
    private static boolean isPrecedence(String type) {
    	if (type.equals(TYPE_PRECEDENCE)) return true;
    	return false;
    }
    ...
    private boolean isPrecedence(Placement p1, Placement p2, boolean firstGoesFirst) {
    	...
    	if (ord1<ord2) {
    		if (firstGoesFirst) {
    			t1 = p1.getTimeLocation(); t2 = p2.getTimeLocation();
    		} else {
    	...
    	return t1.getStartSlots().nextInt()+t1.getLength()<=t2.getStartSlots().nextInt();
    }

    private static boolean sameStudents(String type) {
    	if (type.equals(TYPE_SAME_STUDENTS)) return true;
    	return false;
    }
    
    private static boolean sameInstructor(String type) {
    	if (type.equals(TYPE_SAME_INSTR)) return true;
    	return false;
    }

    public static boolean canShareRooms(String type) {
    	if (type.equals(TYPE_CAN_SHARE_ROOM)) return true;
    	if (type.equals(TYPE_MEET_WITH)) return true;
    	return false;
    }
    ...
    private static boolean sameDays(String type) {
        if (type.equals(TYPE_SAME_DAYS) || type.equals(TYPE_MEET_WITH)) return true;
        return false;
    }
    ...
    private boolean isSatisfiedSeq(Hashtable assignments, boolean considerCurrentAssignments, Set conflicts) {
    	if (conflicts==null)
    		...
    		Set bestConflicts = isSatisfiedRecursive(0, assignments, considerCurrentAssignments, conflicts, new HashSet(), null);
    		if (bestConflicts==null) return false;
    		conflicts.addAll(bestConflicts);
    		return true;
    	}
    }
    
    private Set isSatisfiedRecursive(int idx, Hashtable assignments, boolean considerCurrentAssignments, Set conflicts, Set newConflicts, Set bestConflicts) {
    	if (idx==variables().size() && newConflicts.isEmpty()) return bestConflicts;
    	if (isSatisfiedSeqCheck(assignments, considerCurrentAssignments, conflicts)) {
    		if (bestConflicts==null || bestConflicts.size()>newConflicts.size())
    			return new HashSet(newConflicts);
    		return bestConflicts; 
    	}
    	if (idx==variables().size()) return bestConflicts;
    	bestConflicts = isSatisfiedRecursive(idx+1, assignments, considerCurrentAssignments, conflicts, newConflicts, bestConflicts);
    	Lecture lecture = (Lecture)variables().elementAt(idx);
    	if (assignments!=null && assignments.containsKey(lecture)) return bestConflicts;
        ...
        if (placement==null) return bestConflicts;
        if (conflicts!=null && conflicts.contains(placement)) return bestConflicts;
    ...
    private boolean isSatisfiedSeqCheck(Hashtable assignments, boolean considerCurrentAssignments, Set conflicts) {
        ...
        for (Enumeration e=variables().elements();e.hasMoreElements();) {
            ...
            if (placement==null && considerCurrentAssignments) placement = (Placement)lecture.getAssignment();
            if (placement==null) {
                lengths.addElement(new Integer(((TimeLocation)lecture.timeLocations().firstElement()).getLength()));
        ...
        if (iIsRequired || (!iIsProhibited && iPreference<0)) {
            ...
            while (nrLectures>0) {
                int gap=0;
                while (i<Constants.SLOTS_PER_DAY && res[i]==null) { gap++; i++; }
                if (i==Constants.SLOTS_PER_DAY) break;
                if (!canFill(gap, gapMin, gapMax, lengths)) return false;
                p=res[i];
                i+=res[i].getTimeLocation().getLength();
                nrLectures--;
            }
            ...
            while (nrLectures>0) {
                ...
                if ((gapMin==0 || !canFill(gap, 0, gapMin-1, lengths)) &&
                        (gapMax>=Constants.SLOTS_PER_DAY || !canFill(gap, gapMax+1, Constants.SLOTS_PER_DAY, lengths))) {
                	return false;
                }
                p=res[i];
                i+=res[i].getTimeLocation().getLength();
    ...
    public boolean isSatisfied() {
    	...
    	return (getPreference()<0 && getCurrentPreference()<0) || getPreference()==0 || (getPreference()>0 && getCurrentPreference()==0);
    }
    	...
    	if (lec1.getSchedulingSubpartId().equals(lec2.getSchedulingSubpartId())) {
    		...
    		if (overlap && lec1.getParent()!=null && variables().contains(lec1.getParent()) && lec2.getParent()!=null && variables().contains(lec2.getParent())) {
        		//children overlaps
       			...
       			if (p1!=null && p2!=null && !p1.getTimeLocation().hasIntersection(p2.getTimeLocation())) return false;
        	}
        	
    		if (!overlap && lec1.getChildrenSubpartIds()!=null && lec2.getChildrenSubpartIds()!=null) {
    			//parents not overlap
            	for (Enumeration e1=lec1.getChildrenSubpartIds();e1.hasMoreElements();) {
            		Long subpartId = (Long)e1.nextElement();
            		for (Enumeration e2=lec1.getChildren(subpartId).elements();e2.hasMoreElements();) {
            			Lecture c1 = (Lecture)e2.nextElement();
            			if (c1.getAssignment()==null) continue;
                   		for (Enumeration e3=lec2.getChildren(subpartId).elements();e3.hasMoreElements();) {
                  			Lecture c2 = (Lecture)e3.nextElement();
                  			if (c2.getAssignment()==null) continue;
    ...
    private boolean isSatisfiedPair(Lecture lec1, Placement plc1, Lecture lec2, Placement plc2) {
        if (iIsRequired || (!iIsProhibited && iPreference<0)) {
            if (sameRoom(getType()) || (isBackToBack(getType()) && !isBackToBackTime(getType()))) {
            	if (!plc1.sameRooms(plc2)) return false;
            }
            if (sameStartHour(getType())) {
                if ((plc1.getTimeLocation().getStartSlot()%Constants.SLOTS_PER_DAY) !=
            ...
            if (sameStudents(getType())) {
            	if (JenrlConstraint.isInConflict(plc1,plc2)) return false;
            }
            ...
            if (isBackToBackDay(getType())) {
            	if (sameDays(plc1.getTimeLocation().getDaysArray(), plc2.getTimeLocation().getDaysArray())) return false;
    ...
    public void getInfo(Dictionary info) {
        ...
        info.put("gc"+iId, iType).getDescription()+" (pref="+getDescription()+" ("+iIsRequired+"/"+iIsProhibited+"/"+iPreference+")"+", current="+getCurrentPreference()+", vars=["+varNames+"])");
    }
     */
    
    /*
    public static class GroupConstraintTypes {
        private static Vector sGroupConstraintTypes = null;
        
        static {
        	try {
        		sGroupConstraintTypes = new Vector();
        		for (Iterator i=DistributionType.findAll().iterator();i.hasNext();) {
        			DistributionType type = (DistributionType)i.next();
        			sGroupConstraintTypes.addElement(new GroupConstraintType(type.getRequirementId().intValue(),type.getReference(),type.getLabel()));
        		}
        	} catch (Exception e) {
        	...
        	if (getGroupConstraintType("SAME_STUDENTS")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(20,"SAME_STUDENTS","same students"));
        	if (getGroupConstraintType("SAME_INSTR")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(21,"SAME_INSTR","same instructor"));
        	if (getGroupConstraintType("CAN_SHARE_ROOM")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(22,"CAN_SHARE_ROOM","can share rooms"));
        	if (getGroupConstraintType("PRECEDENCE")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(24,"PRECEDENCE","precedence"));
        	if (getGroupConstraintType("BTB_DAY")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(26,"BTB_DAY","back-to-back day"));
        	if (getGroupConstraintType("MIN_GRUSE(10x1h)")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(27,"MIN_GRUSE(10x1h)","minimize use of 1h groups"));
        	if (getGroupConstraintType("MIN_GRUSE(5x2h)")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(28,"MIN_GRUSE(5x2h)","minimize use of 2h groups"));
        	if (getGroupConstraintType("MIN_GRUSE(3x3h)")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(29,"MIN_GRUSE(3x3h)","minimize use of 3h groups"));
        	if (getGroupConstraintType("MIN_GRUSE(2x5h)")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(30,"MIN_GRUSE(2x5h)","minimize use of 5h groups"));
        	if (getGroupConstraintType("MEET_WITH")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(31,"MEET_WITH","meet together"));
        	if (getGroupConstraintType("NDB_GT_1")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(32,"NDB_GT_1",">1d btw"));
        	if (getGroupConstraintType("CH_NOTOVERLAP")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(33,"NDB_GT_1","ch no overlap"));
        	if (getGroupConstraintType("FOLLOWING_DAY")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(34,"FOLLOWING_DAY","following day"));
        	if (getGroupConstraintType("EVERY_OTHER_DAY")==null)
        		sGroupConstraintTypes.addElement(new GroupConstraintType(35,"EVERY_OTHER_DAY","every other day"));
        }
        ...
        public static GroupConstraintType getGroupConstraintType(String type) {
            for (Enumeration e=sGroupConstraintTypes.elements();e.hasMoreElements();) {
                GroupConstraintType gc = (GroupConstraintType)e.nextElement();
                if (gc.getType().equalsIgnoreCase(type)) return gc;
            }
            return null;
        }

        public static class GroupConstraintType implements Comparable {
            int iId;
            String iType;
            String iDescription;
            ...
            public boolean equals(Object o) {
                if (o==null || !(o instanceof GroupConstraintType)) return false;
                return getId() == ((GroupConstraintType)o;
            }
            
            public int compareTo(Object o) {
                if (o==null || !(o instanceof GroupConstraintType)) return 0;
