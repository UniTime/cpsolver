...
public class CourseRequest extends Request {
     ...
     *                        request priority
     * @param alternative
     *                        true if the request is alternative (alternative
     *                        request can be assigned instead of a non-alternative
     *                        course requests, if it is left unassigned)
     * @param student
     *                        appropriate student
     * @param courses
     *                        list of requested courses (in the correct order --
     *                        first is the requested course, second is the first
     *                        alternative, etc.)
     * @param waitlist
     *                        time stamp of the request if the student can be put on
     *                        a wait-list (no alternative course request will be
     *                        given instead)
     * @param critical
     *                        is the course request is critical for the student in
     *                        order to move forward in their degree
     * @param timeStamp
     *                        request time stamp
     */
    public CourseRequest(long id, int priority, boolean alternative, Student student, java.util.List<Course> courses,
            boolean waitlist, boolean critical, Long timeStamp) {
     ...
     */
    public CourseRequest(long id, int priority, boolean alternative, Student student, java.util.List<Course> courses,
            boolean waitlist, RequestPriority importance, Long timeStamp) {
        super(id, priority, alternative, student);
        iCourses = new ArrayList<Course>(courses);
        for (Course course : iCourses)
            course.getRequests().add(this);
    ...
    /**
     * Create enrollment for the given list of sections. The list of sections needs
     * to be correct, i.e., a section for each subpart of a configuration of one of
     ...
     * 
     * @param assignment
     *                       current assignment (to guess the reservation)
     * @param sections
     *                       selected sections
     * @return enrollment
     */
    ...
    /**
     * Maximal domain size (i.e., number of enrollments of a course request), -1 if
     * there is no limit.
     * 
    ...
    public List<Enrollment> computeEnrollments(Assignment<Request, Enrollment> assignment) {
        ...
        if (getInitialAssignment() != null && getModel() != null && ((StudentSectioningModel) getModel()).isMPP()
                && ((StudentSectioningModel) getModel()).getKeepInitialAssignments()) {
     ...
     * @param assignment
     *                            current assignment
     * @param limitEachConfig
     *                            maximal number of enrollments in each
     *                            configuration
     ...
     * @param priority
     *                           zero for the course, one for the first alternative,
     *                           two for the second alternative
     * @param penalty
     ...
     * @param idx
     *                           index of the subparts (a section of 0..idx-1
     *                           subparts has been already selected)
     * @param availableOnly
     ...
     * @param random
     *                           pick sections in a random order (useful when limit
     *                           is used)
     * @param limit
     *                           when above zero, limit the number of selected
     *                           enrollments to this limit
     * @param ignoreDisabled
     *                           are sections that are disabled for student
     *                           scheduling allowed to be used
     * @param reservations
     *                           list of applicable reservations
     */
    private void computeEnrollments(Assignment<Request, Enrollment> assignment, Collection<Enrollment> enrollments,
            int priority, double penalty, Course course, Config config, HashSet<Section> sections, int idx,
            boolean availableOnly, boolean skipSameTime, boolean selectedOnly, boolean random, int limit) {
        ...
        if (idx == 0) { // run only once for each configuration
            ...
            if (!canOverLimit) {
                if (availableOnly && config.getLimit() >= 0
                        && ConfigLimit.getEnrollmentWeight(assignment, config, this) > config.getLimit())
                    return;
                if (availableOnly && course.getLimit() >= 0
                        && CourseLimit.getEnrollmentWeight(assignment, course, this) > course.getLimit())
                    return;
                if (config.getOffering().hasReservations()) {
                    ...
                    if (!hasReservation && config.getOffering().getTotalUnreservedSpace() < getWeight())
                        return;
                    if (availableOnly && !hasReservation
                            && config.getOffering().getUnreservedSpace(assignment, this) < getWeight())
                        return;
                    if (availableOnly && !hasConfigReservation
                            && config.getUnreservedSpace(assignment, this) < getWeight())
                        return;
        ...
        if (config.getSubparts().size() == idx) {
                ...
                if (availableOnly) {
                    for (Reservation r : getReservations(course)) {
                        ...
                        if (containDisabledSection && !r.isAllowDisabled())
                            continue;
                        enrollments.add(
                                new Enrollment(this, priority, null, config, new HashSet<SctAssignment>(sections), r));
                        canOverLimit = true;
                    }
                }
                if (!canOverLimit) {
                    boolean reservationMustBeUsed = false;
                    reservations: for (Reservation r : (availableOnly ? getSortedReservations(assignment, course)
                            : getReservations(course))) {
                        if (r.mustBeUsed())
                            reservationMustBeUsed = true;
                        if (!r.isIncluded(e))
                            continue;
                        ...
                        if (mustHaveSectionReservation)
                            for (Section s : sections)
                                if (r.getSections(s.getSubpart()) == null && s.getTotalUnreservedSpace() < getWeight())
                        ...
                        enrollments.add(
                                new Enrollment(this, priority, null, config, new HashSet<SctAssignment>(sections), r));
                        if (availableOnly)
                            return; // only one available reservation suffice (the best matching one)
                    }
                    ...
                    if (!(mustHaveReservation || mustHaveConfigReservation || mustHaveSectionReservation)
                        ...
                        enrollments.add(new Enrollment(this, priority, !getReservations(course).isEmpty(), null, config,
                                new HashSet<SctAssignment>(sections), null));
                    }
                }
            }
        } else {
            ...
            if (skipSameTime) {
                ...
                Collections.sort(sectionsThisSubpart,
                        new AssignmentComparator<Section, Request, Enrollment>(assignment));
            }
            ...
            for (Section section : sectionsThisSubpart) {
                ...
                if (getInitialAssignment() != null
                        && (getModel() != null && ((StudentSectioningModel) getModel()).getKeepInitialAssignments())
                ...
                if (!canOverLimit) {
                    ...
                    if (config.getOffering().hasReservations()) {
                        ...
                        if (!hasSectionReservation && section.getTotalUnreservedSpace() < getWeight())
                            continue;
                        if (availableOnly && !hasSectionReservation
                                && section.getUnreservedSpace(assignment, this) < getWeight())
                            continue;
                        if (!hasReservation && reservationMustBeUsed)
                            continue;
                    }
                ...
                if (skipSameTime && section.getTime() != null && !hasChildren && !times.add(section.getTime())
                        ...
                        && (section.getIgnoreConflictWithSectionIds() == null
                                || section.getIgnoreConflictWithSectionIds().isEmpty()))
                    continue;
            ...
            for (Section section : matchingSectionsThisSubpart) {
                ...
                computeEnrollments(assignment, enrollments, priority, penalty + section.getPenalty(), course, config,
                        sections, idx + 1, availableOnly, skipSameTime, selectedOnly, random,
    ...
    public List<Enrollment> getAvaiableEnrollments(Assignment<Request, Enrollment> assignment) {
        ...
        for (Course course : iCourses) {
            for (Config config : course.getOffering().getConfigs()) {
                computeEnrollments(assignment, ret, idx, 0, course, config, new HashSet<Section>(), 0, true, false,
                        false, false, getMaxDomainSize() <= 0 ? -1 : ret.size() + getMaxDomainSize());
            }
            idx++;
        }
    ...
    public List<Enrollment> getSelectedEnrollments(Assignment<Request, Enrollment> assignment, boolean availableOnly) {
        ...
        for (Course course : iCourses) {
            ...
            if (hasChoice)
                for (Config config : course.getOffering().getConfigs()) {
                    computeEnrollments(assignment, enrollments, 0, 0, course, config, new HashSet<Section>(), 0,
                            availableOnly, false, true, false, -1);
                }
            break;
        }
        return enrollments;
    ...
    /**
     * Return all enrollments that are available, pick only the first section of the
     * sections with the same time (of each subpart, {@link Section} comparator is
     * used)
     * 
     * @param assignment
     *                       current assignment
    ...
    public List<Enrollment> getAvaiableEnrollmentsSkipSameTime(Assignment<Request, Enrollment> assignment) {
        ...
        if (getInitialAssignment() != null) {
            ret.add(getInitialAssignment());
            if (getModel() != null && ((StudentSectioningModel) getModel()).isMPP()
                    && ((StudentSectioningModel) getModel()).getKeepInitialAssignments())
                return ret;
        }
        int idx = 0;
        for (Course course : iCourses) {
            ...
            for (LinkedSections link : getStudent().getLinkedSections())
                if (link.getOfferings().contains(course.getOffering())) {
                    skipSameTime = false;
                    break;
                }
            for (Config config : course.getOffering().getConfigs()) {
                computeEnrollments(assignment, ret, idx, 0, course, config, new HashSet<Section>(), 0, true,
                        skipSameTime, false, false, getMaxDomainSize() <= 0 ? -1 : ret.size() + getMaxDomainSize());
            }
            idx++;
        }
        return ret;
    ...
    /**
     * Return all possible enrollments, but pick only the first section of the
     * sections with the same time (of each subpart, {@link Section} comparator is
     * used).
     * 
     * @param assignment
    ...
    public List<Enrollment> getEnrollmentsSkipSameTime(Assignment<Request, Enrollment> assignment) {
        ...
        for (Course course : iCourses) {
            for (Config config : course.getOffering().getConfigs()) {
                ...
                computeEnrollments(assignment, ret, idx, 0, course, config, new HashSet<Section>(), 0, false,
                        skipSameTime, false, false, getMaxDomainSize() <= 0 ? -1 : ret.size() + getMaxDomainSize());
            }
            idx++;
        }
    ...
    /**
     * Return true when the given section is wait-listed (i.e., its choice is among
     * wait-listed choices)
     * 
     * @param section
     *                    given section
    ...
    public boolean isSelected(Section section) {
        for (Choice choice : iSelectedChoices)
            if (choice.sameSection(section) || choice.sameConfiguration(section))
                return true;
        return false;
    }

    /**
     * Return true when the given section has a preference (i.e., there is a
     * matching selection), or when there is a section preference for a different
     * configuration
     * 
     * @param section
     *                    given section
     * @return true if the there is a matching choice for the given section that is
    ...
    /**
     * Return true when the given section is required (i.e., its choice is among
     * required choices, or there are no requirements)
     * 
     * @param section
     *                    given section
    ...
    public boolean isRequired(Section section) {
        ...
        if (hasSubpart && !hasMatchingSection)
            return false;
        // no match, but there are section requirements for a different config -> not
        // satisfied
        if (!hasMatchingConfig && !hasMatchingSection && hasSectionReq)
            return false;
        return true;
    }
    ...
    public String getName() {
        String ret = (isAlternative() ? "A" : "")
                ...
                + (getRequestPriority() != RequestPriority.Normal
                        ? (isWaitlist() ? "(" + getRequestPriority().getAbbreviation() + "w) "
                                : "(" + getRequestPriority().getAbbreviation() + ") ")
                        : isWaitlist() ? "(w) " : "");
        int idx = 0;
        for (Course course : iCourses) {
            if (idx == 0)
    ...
    /**
     * True if the student can be put on a wait-list (no alternative course request
     ...
     * 
     * @param waitlist
     *                     true if the request can be wait-listed
     */
    ...
    /**
     * True if the course request is critical for the student in order to move
     * forward in their degree
     * 
     * @param critical
     *                     true if the request is critical
     */
    @Deprecated
    ...
    /**
     * Minimal penalty (minimum of {@link Offering#getMinPenalty()} among requested
     * courses)
     * 
    ...
    /**
     * Maximal penalty (maximum of {@link Offering#getMaxPenalty()} among requested
     * courses)
     * 
    ...
    /**
     * Estimated bound for this request -- it estimates the smallest value among all
    ...
    public double getBound() {
        ...
        /*
         * if (iCachedBound == null) { iCachedBound =
         * Double.valueOf(-Math.pow(Enrollment.sPriorityWeight, getPriority())
         ...
         * Math.pow(Enrollment.sWaitlistedWeight, (iWaitlistedChoices.isEmpty() ? 0 :
         * 1))
         *
         * // Math.max(Enrollment.sMinWeight,getWeight()) * (getStudent().isDummy() ?
         * Student.sDummyStudentWeight : 1.0)
         * Enrollment.normalizePenalty(getMinPenalty())); } return
         * iCachedBound.doubleValue();
         */
    }
     ...
     * {@link Reservation#compareTo(Assignment, Reservation)}
     * 
     * @param course
     *                   given course
     * @return reservations for this course requests and the given course
    ...
    public TreeSet<Reservation> getSortedReservations(Assignment<Request, Enrollment> assignment, Course course) {
        TreeSet<Reservation> reservations = new TreeSet<Reservation>(
                new AssignmentComparator<Reservation, Request, Enrollment>(assignment));
    ...
    /**
     * Return true when there are restrictions for a course of this course request
     * and the given config does not meet any of them
     */
    ...
    /**
     * Return true when there are restrictions for a course of this course request
     * and the given section does not meet any of them
     */
    ...
    /**
     * Return true when there are restrictions for a course of this course request
     * and the given enrollment does not meet any of them
     */
     ...
     * 
     * @return true if the request is course request and it either has an initial
     *         enrollment
     */
    @Override
    ...
    public float getMinCredit() {
        Float credit = null;
        for (Course course : getCourses()) {
            if (CourseCredit.hasCreditValue() && (credit == null || credit > CourseCredit.getCreditValue()))
                credit = CourseCredit.getCreditValue();
            for (Config config : course.getOffering().getConfigs()) {
