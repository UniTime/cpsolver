...
public class Enrollment extends Value<Request, Enrollment> {
     ...
     * @param priority
     *                                 zero for the course, one for the first
     *                                 alternative, two for the second alternative
     * @param noReservationPenalty
     *                                 when true +1 is added to priority (prefer
     *                                 enrollments with reservations)
     * @param course
     *                                 selected course
     * @param config
     ...
     *                                 used reservation
     */
    public Enrollment(Request request, int priority, boolean noReservationPenalty, Course course, Config config,
            Set<? extends SctAssignment> assignments, Reservation reservation) {
        super(request);
        iRequest = request;
        iConfig = config;
     ...
     *                        used reservation
     */
    public Enrollment(Request request, int priority, Course course, Config config,
            Set<? extends SctAssignment> assignments, Reservation reservation) {
     ...
     */
    public Enrollment(Request request, int priority, Config config, Set<? extends SctAssignment> assignments,
     ...
     *                          current assignment
     * @param onlyAvailable
     *                          use only reservation that have some space left in
     *                          them
     */
        ...
        if (iCourse != null) {
            ...
            for (Reservation reservation : ((CourseRequest) iRequest).getReservations(iCourse)) {
                if (reservation.isIncluded(this)) {
                    if (onlyAvailable && reservation.getContext(assignment).getReservedAvailableSpace(assignment,
                            iRequest) < iRequest.getWeight() && !reservation.canBatchAssignOverLimit())
                        continue;
                    if (best == null || best.getPriority() > reservation.getPriority()) {
                        best = reservation;
                    } else if (best.getPriority() == reservation.getPriority()
                            && best.getContext(assignment).getReservedAvailableSpace(assignment, iRequest) < reservation
                                    .getContext(assignment).getReservedAvailableSpace(assignment, iRequest)) {
                        best = reservation;
                    }
                }
            }
    ...
    public double percentSelectedSameSection() {
        if (!isCourseRequest() || getStudent().isDummy())
            return (getRequest().hasSelection() ? 1.0 : 0.0);
        CourseRequest courseRequest = (CourseRequest) getRequest();
        int nrSelected = 0;
        ...
        sections: for (Section section : getSections()) {
            for (Choice choice : courseRequest.getSelectedChoices()) {
                if (choice.getSubpartId() != null)
                    nrMatching.add(choice.getSubpartId());
    ...
    public double percentSelectedSameConfig() {
        ...
        for (Choice choice : courseRequest.getSelectedChoices()) {
            if (choice.getConfigId() != null) {
                ...
                if (choice.getConfigId().equals(getConfig().getId()))
                    return 1.0;
            }
        }
        ...
        if (!selected.isEmpty()) {
            ...
            sections: for (Section section : getSections()) {
                for (Choice choice : selected) {
                    if (choice.sameOffering(section) && choice.sameInstructionalType(section)
                            && choice.sameTime(section)) {
                        nrInitial++;
                        continue sections;
                    }
     ...
     *                       current assignment
     * @param precise
     *                       if false, distance conflicts and time overlaps are
     *                       ignored (i.e., much faster, but less precise
     *                       computation)
    ...
    public double toDouble(Assignment<Request, Enrollment> assignment, boolean precise) {
        if (precise) {
            ...
            if (model.getStudentQuality() != null)
                return -getRequest().getWeight()
                        * model.getStudentWeights().getWeight(assignment, this, studentQualityConflicts(assignment));
            else
                return -getRequest().getWeight() * model.getStudentWeights().getWeight(assignment, this,
                        distanceConflicts(assignment), timeOverlappingConflicts(assignment));
        } else {
            Double value = (assignment == null ? null : variable().getContext(assignment).getLastWeight());
            if (value != null)
                return -value;
            return -getRequest().getWeight()
                    * ((StudentSectioningModel) variable().getModel()).getStudentWeights().getWeight(assignment, this);
        }
    }
    ...
    public String toString(Assignment<Request, Enrollment> a) {
        ...
        String ret = toDouble(a) + "/" + sDF.format(getRequest().getBound())
                ...
                + (dc == null || dc.isEmpty() ? "" : "/dc:" + dc.size()) + (share <= 0 ? "" : "/toc:" + share);
        if (getRequest() instanceof CourseRequest) {
    ...
    public String toString() {
        ...
        if (getReservation() != null)
            ret = "(r) " + ret;
        if (getRequest().isMPP())
            ret += " [i" + sDF.format(100.0 * percentInitial()) + "/t" + sDF.format(100.0 * percentSameTime()) + "]";
        return ret;
    }
     ...
     * 
     * @return zero for the course, one for the first alternative, two for the
     *         second alternative
     */
    ...
    /**
     * Return adjusted enrollment priority, including priority bump provided by
     * reservations (but ensuring that getting the course without a reservation is
     * still better than getting an alternative)
     * 
     * @return zero for the course, two for the first alternative, four for the
     *         second alternative; plus one when the no reservation penalty applies
     */
    ...
    /**
     * Enrollment limit, i.e., the number of students that would be able to get into
     * the offering using this enrollment (if all the sections are empty)
     * 
     * @return enrollment limit
     */
    public int getLimit() {
        if (!isCourseRequest())
            return -1; // free time requests have no limit
        Integer limit = null;
        for (Section section : getSections())
    ...
    /**
     * Credit of this enrollment (using either {@link Course#getCreditValue()} or
     * {@link Subpart#getCreditValue()} when course credit is not present)
     * 
    ...
    public float getCredit() {
        ...
        if (getAssignments().isEmpty())
            return 0f;
        if ((Course.getCourse()).equals(CourseCredit.hasCreditValue()))
            return getCourse().getCreditValue();
        float subpartCredit = 0f;
        for (Subpart subpart : getConfig().getSubparts()) {
            if (subpart.hasCreditValue())
                subpartCredit += subpart.getCreditValue();
        }
