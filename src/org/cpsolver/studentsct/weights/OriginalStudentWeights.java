...
/**
 * Original weighting that was used before this student weightings model was
 * introduced
 * 
...
public class OriginalStudentWeights implements StudentWeights {
    ...
    public double getWeight(Request request) {
        return Math.pow(iPriorityWeight, request.getPriority()) * (request.isAlternative() ? iAlterativeWeight : 1.0)
    ...
    public double getWeight(Assignment<Request, Enrollment> assignment, Enrollment enrollment) {
        return getWeight(enrollment.getRequest()) * Math.pow(iAltValue, enrollment.getPriority())
    ...
    public double getWeight(Assignment<Request, Enrollment> assignment, Enrollment enrollment,
        ...
        return getWeight(assignment, enrollment)
                * (distanceConflicts == null || distanceConflicts.isEmpty() ? 1.0
                        : Math.pow(iDistConfWeight, distanceConflicts.size()))
    ...
    public double getTimeOverlapConflictWeight(Assignment<Request, Enrollment> assignment, Enrollment enrollment,
        ...
        return Math.min(0.5 * timeOverlap.getShare() / enrollment.getNrSlots(), 0.5)
                * getWeight(assignment, enrollment);
    }
    ...
    public boolean isBetterThanBestSolution(Solution<Request, Enrollment> currentSolution) {
        ...
        return currentSolution.getModel().getTotalValue(currentSolution.getAssignment()) < currentSolution
                .getBestValue();
    }
    ...
    public static void main(String[] args) {
        ...
        new CourseRequest(1l, 0, false, s,
                ToolBox.toList(new Course(1, "A", "1", new Offering(0, "A")),
                        new Course(1, "A", "2", new Offering(0, "A")), new Course(1, "A", "3", new Offering(0, "A"))),
                false, null);
        new CourseRequest(2l, 1, false, s,
                ToolBox.toList(new Course(1, "B", "1", new Offering(0, "B")),
                        new Course(1, "B", "2", new Offering(0, "B")), new Course(1, "B", "3", new Offering(0, "B"))),
                false, null);
        new CourseRequest(3l, 2, false, s,
                ToolBox.toList(new Course(1, "C", "1", new Offering(0, "C")),
                        new Course(1, "C", "2", new Offering(0, "C")), new Course(1, "C", "3", new Offering(0, "C"))),
                false, null);
        new CourseRequest(4l, 3, false, s,
                ToolBox.toList(new Course(1, "D", "1", new Offering(0, "D")),
                        new Course(1, "D", "2", new Offering(0, "D")), new Course(1, "D", "3", new Offering(0, "D"))),
                false, null);
        new CourseRequest(5l, 4, false, s,
                ToolBox.toList(new Course(1, "E", "1", new Offering(0, "E")),
                        new Course(1, "E", "2", new Offering(0, "E")), new Course(1, "E", "3", new Offering(0, "E"))),
                false, null);
        new CourseRequest(6l, 5, true, s,
                ToolBox.toList(new Course(1, "F", "1", new Offering(0, "F")),
                        new Course(1, "F", "2", new Offering(0, "F")), new Course(1, "F", "3", new Offering(0, "F"))),
                false, null);
        new CourseRequest(7l, 6, true, s,
                ToolBox.toList(new Course(1, "G", "1", new Offering(0, "G")),
                        new Course(1, "G", "2", new Offering(0, "G")), new Course(1, "G", "3", new Offering(0, "G"))),
                false, null);

        Assignment<Request, Enrollment> assignment = new DefaultSingleAssignment<Request, Enrollment>();
        Placement p = new Placement(null, new TimeLocation(1, 90, 12, 0, 0, null, null, new BitSet(), 10),
                new ArrayList<RoomLocation>());
        ...
        for (Request r : s.getRequests()) {
            ...
            for (int i = 0; i < cr.getCourses().size(); i++) {
                ...
                toc.add(new TimeOverlapsCounter.Conflict(s, 3, e, sections.iterator().next(), e,
                ...
                switch (c.getType().getType()) {
                    case REQUEST:
                        if (enrollment.isCourseRequest())
                            ...
                            penalty += getWeight(assignment, other) * iQalityWeights[c.getType().ordinal()]
                                    * c.getWeight(other);
                        break;
                    case BOTH:
                        penalty += base * iQalityWeights[c.getType().ordinal()] * c.getWeight(enrollment);
    ...
    public abstract class ConflictType {
        public abstract double getWeight(Assignment<Request, Enrollment> assignment, Enrollment enrollment1,
                Enrollment enrollment2, int weightIndex, StudentQuality.Conflict conflict);
    }

    public class BothConflictType extends ConflictType {
        @Override
        public double getWeight(Assignment<Request, Enrollment> assignment, Enrollment enrollment1,
                Enrollment enrollment2, int weightIndex, StudentQuality.Conflict conflict) {
            if (enrollment1 == null || enrollment1.getRequest() == null)
                return 0.0;
            return ((getWeight(assignment, enrollment1, enrollment2, weightIndex, conflict)
                    * iQalityWeights[weightIndex]) * (conflict.getWeight(enrollment1)));

        }
    }

    public class RequestConflictType extends ConflictType {
        @Override
        public double getWeight(Assignment<Request, Enrollment> assignment, Enrollment enrollment1,
                Enrollment enrollment2, int weightIndex, StudentQuality.Conflict conflict) {
            if (enrollment1 == null || enrollment1.getRequest() == null || !enrollment1.isCourseRequest())
                return 0.0;
            return (getWeight(assignment, enrollment1, enrollment2, weightIndex, conflict) * iQalityWeights[weightIndex]
                    * conflict.getWeight(enrollment1));
        }
    }

    public class LowerConflictType extends ConflictType {
        @Override
        public double getWeight(Assignment<Request, Enrollment> assignment, Enrollment enrollment1,
                Enrollment enrollment2, int weightIndex, StudentQuality.Conflict conflict) {
            if (enrollment1.getRequest().getPriority() < enrollment2.getRequest().getPriority()) {
                return getWeight(assignment, enrollment1, enrollment2, weightIndex, conflict)
                        * iQalityWeights[weightIndex] * conflict.getWeight(enrollment2);
            } else {
                return getWeight(assignment, enrollment1, enrollment2, weightIndex, conflict)
                        * iQalityWeights[weightIndex] * conflict.getWeight(enrollment1);
            }
    ...
    public class HigherConflictType extends ConflictType {
        @Override
        public double getWeight(Assignment<Request, Enrollment> assignment, Enrollment enrollment1,
                Enrollment enrollment2, int weightIndex, StudentQuality.Conflict conflict) {
            if (enrollment1.getRequest().getPriority() > enrollment2.getRequest().getPriority()) {
    ...
    public double getStudentQualityConflictWeight(Assignment<Request, Enrollment> assignment, Enrollment enrollment1,
            Enrollment enrollment2, StudentQuality.Conflict conflict) {
        ConflictType conflictType;
        switch (conflict.getType().getType()) {
            case BOTH:
                conflictType = new BothConflictType();
                break;
            case REQUEST:
                conflictType = new RequestConflictType();
                break;
            case LOWER:
                conflictType = new LowerConflictType();
                break;
            case HIGHER:
                conflictType = new HigherConflictType();
                break;
            default:
                return 0.0;
        }
        return conflictType.getWeight(assignment, conflict.getE1(), conflict.getE2(), conflict.getType().ordinal(),
                conflict);
    }

    @Override
    public double getStudentQualityConflictWeight(Assignment<Request, Enrollment> assignment, Enrollment enrollment,
            Conflict conflict) {
        throw new UnsupportedOperationException("Unimplemented method 'getStudentQualityConflictWeight'");
    }
